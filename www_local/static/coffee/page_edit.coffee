...
$ ->
    ...
    iframe_add_iframe_pin = (dom_sequence)->
        ...
        iframe_add_iframe_pin_timout = setTimeout ()->
            ...
            ,1000
    iframe_add_comments_area_timeout = null
    iframe_add_comments_area = (dom_sequence,block_id)->
        clearTimeout iframe_add_comments_area_timeout
        iframe_add_comments_area_timeout = setTimeout ()->
                if $("##{dom_sequence}").length < 1
                    return iframe_add_comments_area(dom_sequence,block_id)
                if $(".comments_area[data-block=#{block_id}]").length>0
                    alert "already loaded in page"
                    return
                iframe_comments_area = """
                <div class="comments_area" data-block="#{block_id}">
                    <div><textarea class="comment_content" placeholder="Add some comment..."></textarea><button class="comment_submit">submit</button></div>
                    <div class="comments"></div>
                </div>
                <script>
                $(window).on("load",function(){
                    comment_load("#{block_id}")
                })
                </script>
                """
                $("##{dom_sequence}").find(".iframe_html").val(iframe_comments_area)
                $("##{dom_sequence}").find(".iframe_save").click()
            ,1000
    $("body").on "click",".comment_entity_line_tools>.load_card",(evt)->
        comments_area_block = $(this).parents(".comment_entity_line").attr("data-block")
        dom_type_old = "comments_area"
        ...
        $.ajax
            ...
            success:(data)->
                ...
                if data.info == "error"
                    alert data.about
                if data.info == "ok"
                    if dom_type_old in ["comments_area"]
                        iframe_add_comments_area(data.dom_sequence,comments_area_block)
            error:(data)->
                console.log data
        ...
        $.ajax
            ...
            success:(data)->
                ...
                if data.info == "ok"
                    ...
                    else if dom_type_old in ["comments_area"]
                        iframe_add_comments_area(data.dom_sequence)
            error:(data)->
                console.log data
        ...
        $.ajax
            ...
            success:(data)->
                ...
                if data.info == "ok"
                    ...
                    <div class="comment_entity_line" data-block="#{data.comment_entity}">
